...
// Populate base
        ...
        // 🟢 FIX 1: Set the Requested Time Label
        modalTimeRequestedLabel.setText("Requested at: " + record.getTimeRequested());

        // 🟢 FIX 2: Call getReasonForLeaving() to get the FULL string with all the pipes
        ...
        if (rawData != null && rawData.contains("|")) {
            ...
            for (String part : parts) {
                ...
                if (p.startsWith("Destination:")) dest = p.replaceFirst("Destination:", "").trim();
                if (p.startsWith("Reason:")) reas = p.replaceFirst("Reason:", "").trim();
                if (p.startsWith("Est. Out:")) estOut = p.replaceFirst("Est. Out:", "").trim();
                if (p.startsWith("Est. In:")) estIn = p.replaceFirst("Est. In:", "").trim();
            }
        }
    ...
    private void handleApprove() {
        // 🟢 3. FIXED: Strict Title Case to match Supabase enum
