...
public class PassSlipService {
    ...
    public IssuanceResult issuePassSlip(String employeeId, String destination, String reason) {
        ...
        LocalDateTime timeRequested = LocalDateTime.now(); // We use this now instead of timeOut

        // 9 Parameters to match your updated PassSlipDTO constructor
        PassSlipDTO dto = new PassSlipDTO(
                slipId,
                employeeId,
                destination.trim(),
                reason.trim(),
                timeRequested, // 1. Time Requested
                null,          // 2. Time Out (null until approved)
                null,          // 3. Time In
                null,          // 4. Total Duration
                "FOR_APPROVAL" // 5. Initial Status
        );
        try {
        ...
        PassSlipIssuedEvent event = new PassSlipIssuedEvent(slipId, employeeId, timeRequested);
        eventPublisher.publish(event);

        LOGGER.info(String.format(
                "Pass slip [%s] successfully issued for employee [%s] at [%s]. Event published.",
                slipId, employeeId, timeRequested
        ));
